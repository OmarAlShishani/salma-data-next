...
export const createSmalltalkTesting = async (req: any, res: Response) => {
    const { user_type, id: userId } = req.user;
    const { smalltalkId, text } = req.body;
    if (!text) {
        return res.status(201).json({
            status_code: 400,
            message: 'Please Fill Small Talk Testing Name text box',
            isSuccess: false,
            results: null,
        });
    }
    ...
    const checkName = await context.prisma.smalltalk_testing_phrases.findFirst({
        where: {
            normalized_text: {
    ...
    if (checkName) {
        return res.status(201).json({
            status_code: 400,
            message: 'This small talks testing already exists',
            isSuccess: false,
            results: null,
        });
    }
    return await context.prisma.smalltalk_testing_phrases
        .create({
            data: {
                text,
                smalltalks: {
...
export const getSmalltalkTesting = async (req: any, res: Response) => {
    const { user_type, id: userId } = req.user;
    ...
    result = await context.prisma.smalltalk_testing_phrases
        .findMany({
            where,
            orderBy: {
                id: 'desc',
    ...
    count = await context.prisma.smalltalk_testing_phrases
        .aggregate({
            where,
            _count: {
                id: true,
...
export const getSmalltalkTestingById = async (
    req: Request,
    res: Response
) => {};
    ...
    const result = await Promise.all(
        file.map(
            async ({ id, text }: { id: number; text: string }, i: number) => {
                ...
                const normalized_text = checkTextAndReWrite(text);
                const find =
                    await context.prisma.smalltalk_testing_phrases.findFirst({
                        where: {
...
export const exportXlsxFile = async (req: any, res: Response) => {
    ...
    const result = await context.prisma.smalltalk_testing_phrases
        .findMany({
            where: {
                active: { equals: true },
                smalltalk_id: { equals: parseInt(smalltalkId) },
...
export const deleteSmalltalkTesting = async (req: any, res: Response) => {
    const { user_type, id: userId } = req.user;
    const { id, smalltalkId } = req.params;
    if (!id) {
        return res.status(201).json({
            status_code: 400,
            message: 'Small talk Testing Id not Fount',
            isSuccess: false,
            results: null,
        });
    }
    ...
    const result = await context.prisma.smalltalk_testing_phrases
        .update({
            ...
            data: {
                ...
                user_smalltalk_testing_phrases_deleted_byTouser: {
                    connect: {
                        id: userId,
                    },
                },
...
export const updateSmalltalkTesting = async (req: any, res: Response) => {
    const { user_type, id: userId } = req.user;
    const { id, smalltalkId } = req.params;
    const { data } = req.body;
    if (!id) {
    ...
    const result = await context.prisma.smalltalk_testing_phrases
        .update({
            ...
            data: {
                ...
                user_smalltalk_testing_phrases_modified_byTouser: {
                    connect: {
                        id: userId,
                    },
                },
